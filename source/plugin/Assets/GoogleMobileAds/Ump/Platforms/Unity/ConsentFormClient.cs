...
namespace GoogleMobileAds.Ump.Unity
{
    internal class ConsentFormClient : IConsentFormClient
    {
        ...
        private static readonly DummyFormBehaviour FormBehaviour =
        ...
        public void Show(Action<FormError> onDismiss)
        {
            if (prefabForm != null)
            {
                dummyForm = FormBehaviour.ShowForm(prefabForm, new Vector3(0, 0, 1));
                CreateButtonBehavior();
                AddClickBehavior(dummyForm, onDismiss);
                Debug.Log("Consent Form Shown.");
                FormBehaviour.PauseGame();
            }
            else
            {
        ...
        {
            buttonBehaviour = dummyForm.AddComponent<ButtonBehaviour>();
        }
        ...
        {
            FormBehaviour.DestroyForm(dummyForm);
            prefabForm = null;
        }
    }
}
